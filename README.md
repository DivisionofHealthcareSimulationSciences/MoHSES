...
MoHSES is an open-source multiplatform library, but modules that depend on pistache are not available on non-*nix platforms.  The following development tools are required to get started.
...
MoHSES depends on several opensource libraries which act as thirdparty dependencies.  Our build system includes a method for pulling and downloading these libraries if you must build them from source, but often installing them through your systems package system will be preferred. 
...
| pistache | v0.1.1 commit a68ad09 | [https://github.com/pistacheio/pistache.git] | False |

## Setting up a build environment

MoHSES can be built on windows and macos, but its primarily supported on debian 
...
option 2 we will instead use apt to install the bulk of the utilities. Both options are equally viable.
...
and install all dependencies that are not detected. While the script can build all dependencies. 
We highly recommend you install libssl, libboost, libz, and xsdcxx using your package management system.
When cross compiling consider that xsdcxx has two rolls the first is to run a native code generator and 
the second is to access cxx headers at build time. So you will need both packages or to make the native 
headers available to the cross compiler sysroot via symlink or copy.  In this example we build all but the
...
MoHSES has a very specific dependency for fastdds and biogears. In this example we install all
...
user had placed the dependencies in `/home/mohses/usr` passing `-D CMAKE_PREFIX_PATH=/home/amm/usr` to the cmake
configuration would allow it to find all the dependencies.  For cross compilation set `-D CMAKE_FIND_ROOT_PATH=/home/mohses/cross-sysroot`
...
thirdparty installs across multiple roots. Use a ":" to separate directories and a "/" for all paths even if you are on windows.

```bash
    $ cmake -G Ninja -D CMAKE_PREFIX_PATH=/home/mohses/usr/ \
...
CMake makes targeting non native relatively easy. To successfully cross compile you will need
...
overview of cross-compiling with CMake. If you have a specific use case such as targeting Android, iOS or embedded targets. See your devices vendor documentation for more information.
...
You can install MoHSES locally or as a system process. CMake provides an install target which will move files according the CMAKE_INSTALL_PREFIX.  CMake considers the PREFIXES /usr/local and /usr to be SYSTEM installs. In System installs the LOG, RESOURCE, and BIN direcotries are different then local installs. A default ADMIN install will assume the following
